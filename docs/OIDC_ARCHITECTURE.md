...
- `OidcProviderConfigService`: OIDC配置管理服务

### 基础设施
- 策略模式：`OidcProviderStrategy`接口和具体实现
- 动态策略工厂：`DynamicOidcStrategyFactory`
- 数据持久化：`OidcProviderConfigEntity`, `OidcProviderConfigRepositoryImpl`

## 数据库设计

### 服务商注册表 (boat_oidc_provider_registry)

该表定义在 `db/schema/schema-postgresql.sql` (PostgreSQL) 和 `db/schema/schema.sql` (H2) 中。
    ...
    public DynamicOidcStrategyFactory(List<OidcProviderStrategy> strategyList) {
        this.strategies = strategyList.stream()
                .collect(Collectors.toMap(
                    strategy -> AuthProvider.of(strategy.getClass().getSimpleName().replace("Strategy", "")).getValue(),
                    strategy -> strategy
                ));
    }
    
    public OidcProviderStrategy getStrategy(String providerCode) {
        OidcProviderStrategy strategy = strategies.get(providerCode.toUpperCase());
        if (strategy == null) {
